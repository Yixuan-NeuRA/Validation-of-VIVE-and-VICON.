function error = Marker_Error_LToe(params, rw_LToe_VIVE, rx_LToe_VIVE, ry_LToe_VIVE, rz_LToe_VIVE, pos_LToe_Original, time_column, z_LToe, x_LToe, y_LToe, t_corr)
...
% z_LToe, x_LToe, y_LToe - VICON position data
...
pos_Transformed = zeros(length(rw_LToe_VIVE), 3);
for i = 1:length(rw_LToe_VIVE)
    % LToe
    q = [rw_LToe_VIVE(i), rx_LToe_VIVE(i), ry_LToe_VIVE(i), rz_LToe_VIVE(i)];
    R = quat2rotm(q);
    pos_Transformed(i,:) = (R * (pos_LToe_Original(i,:) + local_offset)')';
end
...
for i = 1:length(unique_seconds)
    ...
    current_x_LToe = pos_Transformed(indices, 1);
    current_y_LToe = pos_Transformed(indices, 2);
    current_z_LToe = pos_Transformed(indices, 3);
    ...
    interp_x_LToe = interp1(t_original, current_x_LToe, t_interp, 'pchip');
    interp_y_LToe = interp1(t_original, current_y_LToe, t_interp, 'pchip');
    interp_z_LToe = interp1(t_original, current_z_LToe, t_interp, 'pchip');
    
    % Store interpolated data
    add_index = (i-1)*100+1 : i*100;
    all_data_x(add_index,1) = interp_x_LToe';
    all_data_y(add_index,1) = interp_y_LToe';
    all_data_z(add_index,1) = interp_z_LToe';
...
VICON_points = [x_LToe(indexVICON), y_LToe(indexVICON), z_LToe(indexVICON)];
